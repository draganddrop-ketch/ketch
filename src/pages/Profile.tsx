...
import { User, ShoppingCart, Settings, Trash2, Share2, ZoomIn, Edit2, CreditCard, X, Package } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: bgColor, color: textColor }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <h1 className="text-4xl font-bold mb-8 text-[#34d399] tracking-wider font-mono">MY DASHBOARD</h1>
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          
          <div className="lg:col-span-1">
            <div className="p-6 rounded-lg border sticky top-24" style={{ ...borderStyle, backgroundColor: 'rgba(255,255,255,0.03)' }}>
              <div className="flex items-center justify-between mb-6"><h2 className="text-xl font-bold text-[#34d399]">ID CARD</h2><User className="opacity-50"/></div>
              ...
              <button onClick={() => setActiveTab('SETTINGS')} className="w-full mt-6 bg-[#34d399] hover:bg-[#2ebb88] text-black font-bold py-3 rounded transition-colors"><Edit2 className="inline w-4 h-4 mr-2"/>EDIT PROFILE</button>
            </div>
          </div>
            ...
            <div className="border rounded-lg overflow-hidden min-h-[600px]" style={borderStyle}>
              <div className="flex border-b" style={{ borderColor: borderStyle.borderColor }}>
                {(['ORDER_HISTORY', 'SAVED_DESIGNS', 'SETTINGS'] as TabType[]).map((tab) => (
                  <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-1 py-4 font-bold text-sm tracking-wider transition-colors ${activeTab === tab ? 'bg-[#34d399] text-black' : 'hover:bg-white/5'}`} style={activeTab !== tab ? { color: textColor } : {}}>{tab.replace('_', ' ')}</button>
                ))}
              </div>
              <div className="p-6">
                
                {activeTab === 'ORDER_HISTORY' && (
                  <div className="space-y-6">{orders.length === 0 ? <div className="text-center py-12 opacity-50">No Orders</div> : orders.map(order => (<div key={order.id} className="border rounded-lg p-4 flex gap-4" style={borderStyle}><div className="w-32 h-32 bg-zinc-900 rounded overflow-hidden flex items-center justify-center">{order.custom_image_url ? <img src={order.custom_image_url} className="w-full h-full object-cover"/> : <Package className="text-gray-600"/>}</div><div><p className="font-bold">{formatDate(order.order_date)}</p><p className="text-[#34d399] font-bold">₩{order.total_price.toLocaleString()}</p><span className={`text-xs px-2 py-1 border rounded ${getStatusColor(order.status)}`}>{order.status}</span></div></div>))}</div>
                )}
                    ...
                    {savedDesigns.length === 0 ? <div className="text-center py-12 opacity-50">No Designs</div> : (
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {savedDesigns.map((design) => (
                          ...
                          <div key={design.id} className="bg-white border border-gray-200 hover:border-[#34d399] transition-all rounded-lg overflow-hidden group shadow-sm">
                            ...
                            <div className="p-4">
                              ...
                              <p className="text-[#34d399] font-bold mb-4">₩{calculateTotalPrice(design).toLocaleString()}</p>
                              <div className="grid grid-cols-2 gap-2">
                                <button onClick={() => handleAddToCart(design)} className="bg-[#34d399] hover:bg-[#2ebb88] text-black font-bold py-2 rounded text-xs flex items-center justify-center gap-1"><ShoppingCart size={12}/> CART</button>
                ...
                {activeTab === 'SETTINGS' && editForm && (
                  <div className="max-w-2xl mx-auto space-y-6 py-4">
                    ...
                    <div className="space-y-4">
                      ...
                      <div><label className="block text-sm opacity-50 mb-1">이름</label><input value={editForm.full_name || ''} onChange={e => setEditForm({...editForm, full_name: e.target.value})} className="w-full bg-transparent border p-3 rounded outline-none focus:border-[#34d399]" style={{ ...borderStyle, color: textColor }}/></div>
                      <div><label className="block text-sm opacity-50 mb-1">휴대폰 번호</label><input value={editForm.phone || ''} onChange={e => setEditForm({...editForm, phone: e.target.value})} className="w-full bg-transparent border p-3 rounded outline-none focus:border-[#34d399]" style={{ ...borderStyle, color: textColor }} placeholder="010-0000-0000"/></div>
                      <div>
                        ...
                        <div className="flex gap-2 mb-2">
                          ...
                          <button onClick={() => setIsAddressModalOpen(true)} className="px-4 rounded text-sm font-bold flex items-center gap-2 bg-[#34d399] text-black">주소 검색</button>
                        </div>
                        ...
                        <input value={editForm.address_detail || ''} onChange={e => setEditForm({...editForm, address_detail: e.target.value})} className="w-full bg-transparent border p-3 rounded outline-none focus:border-[#34d399]" style={{ ...borderStyle, color: textColor }} placeholder="상세주소"/>
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                        <div><label className="block text-sm opacity-50 mb-1">생년월일</label><input type="date" value={editForm.birthday || ''} onChange={e => setEditForm({...editForm, birthday: e.target.value})} className="w-full bg-transparent border p-3 rounded outline-none focus:border-[#34d399]" style={{ ...borderStyle, color: textColor }}/></div>
                        <div><label className="block text-sm opacity-50 mb-1">성별</label><div className="flex gap-4 py-3"><label className="flex items-center gap-2 cursor-pointer"><input type="radio" checked={editForm.gender === '남성'} onChange={() => setEditForm({...editForm, gender: '남성'})} className="accent-[#34d399]"/> 남성</label><label className="flex items-center gap-2 cursor-pointer"><input type="radio" checked={editForm.gender === '여성'} onChange={() => setEditForm({...editForm, gender: '여성'})} className="accent-[#34d399]"/> 여성</label></div></div>
                      </div>
                      <label className="flex items-center gap-2 mt-4 cursor-pointer select-none"><input type="checkbox" checked={editForm.is_agreed_marketing || false} onChange={e => setEditForm({...editForm, is_agreed_marketing: e.target.checked})} className="w-5 h-5 accent-[#34d399]"/><span className="opacity-70">마케팅 정보 수신 동의 (이메일/문자)</span></label>
                    </div>
                    <div className="pt-6 flex justify-end"><button onClick={handleUpdateProfile} className="bg-[#34d399] hover:bg-[#2ebb88] text-black font-bold py-3 px-8 rounded flex items-center gap-2"><Save size={18}/> 저장하기</button></div>
                  </div>
                )}
              </div>
            </div>
      ...
      {showDetailModal && selectedDesign && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowDetailModal(false)}>
          <div className="bg-gray-900 border-2 border-[#34d399] rounded-lg overflow-hidden w-full max-w-6xl max-h-[95vh] flex flex-col md:flex-row" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="w-full md:w-[400px] p-8 flex flex-col bg-gray-900 overflow-y-auto">
               ...
               <div className="flex-1 mb-6"><h4 className="text-sm font-bold text-gray-500 mb-3">Components</h4><div className="space-y-2">{selectedDesign.design_data?.items?.map((item:any, i:number) => (<div key={i} className="flex items-center gap-3 bg-black/50 p-2 rounded"><img src={item.image} className="w-10 h-10 object-contain"/><span className="text-white text-sm font-bold flex-1 truncate">{item.name}</span><span className="text-[#34d399] font-bold">₩{(item.price||0).toLocaleString()}</span></div>))}</div></div>
               <div className="pt-4 border-t border-gray-700"><div className="flex justify-between items-end mb-4"><span className="text-gray-400 font-bold">TOTAL</span><span className="text-3xl font-black text-[#34d399]">₩{calculateTotalPrice(selectedDesign).toLocaleString()}</span></div><button onClick={() => handleAddToCart(selectedDesign)} className="w-full bg-[#34d399] hover:bg-[#2ebb88] text-black font-black py-4 rounded text-lg mb-2">ADD TO CART</button><button onClick={()=>handleOrder(selectedDesign)} className="w-full bg-cyan-600 hover:bg-cyan-500 text-black font-bold py-3 rounded text-lg">ORDER NOW</button></div>
            </div>
          </div>
        </div>
      )}
