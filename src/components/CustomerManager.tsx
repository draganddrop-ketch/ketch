        ...
        <div className="flex-1 overflow-auto p-6">
          <div className="bg-white border border-gray-200 rounded-lg shadow-sm">
            <table className="w-full text-left text-sm">
              <thead className="bg-gray-50 text-gray-500 border-b border-gray-200"><tr><th className="py-3 px-4">이름</th><th className="py-3 px-4">아이디</th><th className="py-3 px-4">이메일</th><th className="py-3 px-4">휴대폰 번호</th><th className="py-3 px-4">등급</th><th className="py-3 px-4">권한</th><th className="py-3 px-4">가입일</th><th className="py-3 px-4">관리</th></tr></thead>
              <tbody className="divide-y divide-gray-100">{filteredCustomers.map(customer => (<tr key={customer.id} className="hover:bg-gray-50"><td className="py-4 px-4 font-bold text-gray-800">{customer.full_name}</td><td className="py-4 px-4 text-gray-600">{customer.username || '-'}</td><td className="py-4 px-4 text-gray-600">{customer.email}</td><td className="py-4 px-4 text-gray-600">{customer.phone || '-'}</td><td className="py-4 px-4"><span className="bg-gray-100 text-gray-600 px-2 py-1 rounded text-xs">{customer.grade || '일반'}</span></td><td className="py-4 px-4"><span className={`px-2 py-1 rounded text-xs ${customer.role === 'admin' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600'}`}>{customer.role === 'admin' ? '관리자' : '일반'}</span></td><td className="py-4 px-4 text-gray-500">{customer.created_at?.split('T')[0]}</td><td className="py-4 px-4"><button onClick={() => setSelectedCustomer(customer)} className="text-blue-600 hover:underline">상세</button></td></tr>))}</tbody>
            </table>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex justify-end">
          <div className="w-full max-w-xl bg-white h-full shadow-2xl overflow-y-auto p-8 animate-slide-in-right">
            ...
            <div className="space-y-8">
              ...
              <section><h3 className="font-bold text-lg mb-4 border-b pb-2">활동 정보</h3><div className="grid grid-cols-3 gap-y-4 text-sm items-center"><div className="text-gray-500">등급</div><div className="col-span-2"><select value={selectedCustomer.grade || '일반'} onChange={(e) => handleUpdateCustomer(selectedCustomer.id, { grade: e.target.value })} className="border rounded p-2"><option value="일반">일반</option><option value="실버">실버</option><option value="골드">골드</option></select></div><div className="text-gray-500">권한</div><div className="col-span-2"><select value={selectedCustomer.role || 'user'} onChange={(e) => handleUpdateCustomer(selectedCustomer.id, { role: e.target.value })} className="border rounded p-2"><option value="user">일반 사용자</option><option value="admin">관리자</option></select><p className="text-xs text-gray-400 mt-1">관리자로 변경하면 관리자 페이지 접근 권한이 부여됩니다.</p></div><div className="text-gray-500">관리자 메모</div><div className="col-span-2"><textarea className="w-full border rounded p-2 h-24" value={selectedCustomer.memo || ''} onChange={(e) => setSelectedCustomer({...selectedCustomer, memo: e.target.value})} onBlur={(e) => handleUpdateCustomer(selectedCustomer.id, { memo: e.target.value })} placeholder="고객 특이사항 메모 (자동저장)"/></div></div></section>
            </div>
          </div>
        </div>
      )}
